...
const fallbackTimelineData = [
  ...
  { year: "2019", text: "Second generation 'Proline' series of fittings & tubing" },
  { year: "2021", text: "Second generation 'Proline' series of fittings & tubing" },
  { year: "2022", text: "Pneumatic cylinder assembly  unit set up for P1D range of ISO 15552 cylinders to grow presence in the pneumatic automation market" },
  { year: "2022", text: "Parker pneumatic automation range of products added in scope of offerings " },
  { year: "2022", text: "Parker industrial air filtration & precision fluidics division products added in distribution range " },
  { year: "2023", text: "Operations set up for multi-axes metal tube bending & end-forming to offer complete BOM of high & low pressure fluid connection assemblies to OEMs" },
  { year: "2024", text: "Exports grow to 15% of business volume" },
  { year: "2024", text: "New unit with additional 100,000 sq.ft. built up space acquired in close proximity to existing unit " },
  { year: "2025", text: "Project awarded for piping fabrication & installation at India’s first semi-conductor chip fabrication plant" },
];

// Fallback story content for heading description
const fallbackStoryDescription = "Laminar Industries (formerly Legris India Pvt.Ltd.) serves diversified industries as a fluid connection solution provider across its key product groups in pneumatics, hydraulics & industrial piping.";

// Transform API data for timeline (year + description)
const transformTimeline = (apiData) => {
  if (!Array.isArray(apiData) || apiData.length === 0) return null;
  // Map API data to timeline format, sorted by year (title field contains year)
  return apiData
    .map(item => ({
      year: item.title || "",
...
function OurStory() {
  ...
  const { data: apiTimelineData } = useApiData(
    "/api/home?type=aboutus-story",
    null,
    transformTimeline
  );
  ...
  // Use API data if available, otherwise use fallback
  const timelineData = apiTimelineData || fallbackTimelineData;

  return (
    <section className="relative w-full bg-gray-900 text-white py-16 px-8 flex items-center justify-center">
      <div className="w-full max-w-7xl">
        {/* Header */}
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-4">Our Story</h2>
          <p className="text-sm md:text-base max-w-3xl mx-auto text-gray-300">
            {fallbackStoryDescription}
          </p>
        </div>
